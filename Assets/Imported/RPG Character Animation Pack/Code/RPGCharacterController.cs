...
public class RPGCharacterController : MonoBehaviour 
{
	...
	{
		if(inputVec != Vector3.zero && !isStrafing && !isRolling && !isBlocking && rpgCharacterState != RPGCharacterState.CLIMBING)
		{
	...
	void Blocking()
	{
		if(Input.GetAxisRaw("TargetBlock") < -.1 && canAction && isGrounded)
		{
			if(!isBlocking)
			{
